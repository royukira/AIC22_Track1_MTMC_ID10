...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key, model_name, patch_size=0, is_training=False):
    if os.path.isfile(pretrained_weights):
        ...
        if is_training and ('rit' in model_name) and checkpoint_key == "student":
            # remove `module.backbone.pos_embed`
            state_dict = {k:v for k, v in state_dict.items() if "module.backbone.pos_embed" not in k}
            # remove `module.backbone.patch_embed`
            state_dict = {k:v for k, v in state_dict.items() if "module.backbone.patch_embed" not in k}
        
        if is_training and ('rit' in model_name) and checkpoint_key == "teacher":
            # remove `backbone.pos_embed`
            state_dict = {k:v for k, v in state_dict.items() if "backbone.pos_embed" not in k}
            # remove `backbone.patch_embed`
            state_dict = {k:v for k, v in state_dict.items() if "backbone.patch_embed" not in k}
...
def restart_from_checkpoint(ckp_path, run_variables=None, is_training=True, **kwargs):
    """
    ...
    for key, value in kwargs.items():
        if key in checkpoint and value is not None:
            try:
                ...
                if is_training is False:
                    # remove `head`
                    static_dict = {k:v for k, v in checkpoint[key].items() if "head" not in k}
                
                msg = value.load_state_dict(static_dict, strict=False)
                ...
                try:
                    ...
                    if is_training is False:
                        # remove `head`
                        static_dict = {k:v for k, v in checkpoint[key].items() if "head" not in k}
                    msg = value.load_state_dict(static_dict)
