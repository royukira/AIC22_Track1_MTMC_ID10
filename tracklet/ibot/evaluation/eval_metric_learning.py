...
from loss.circle_loss import convert_label_to_similarity
...
def eval_linear(args):
    ...
    if args.angular_penalty is None:
        use_angular_loss = False
    else:
        use_angular_loss = True

    if args.circle or args.triplet:
    ...
    for pm in args.permutes:
        ...
        linear_classifier = LinearClassBlock(feat_dim, args.num_labels, return_f=use_similarity_loss, num_bottleneck=args.bottleneck_dim)
        linear_classifier = linear_classifier.cuda()
        linear_classifier = nn.parallel.DistributedDataParallel(linear_classifier, device_ids=[args.gpu])
        linear_classifiers.append(linear_classifier)
    ...
    # Loss
    # ------------------
    criterion_dict = get_loss(args, feature_dim=args.bottleneck_dim)
    loss_type = ""
    if args.circle:
        if use_angular_loss:
            loss_type = "circle/angular"
        else:
            loss_type = "circle/ce"
        criterion_similarity = criterion_dict['similarity']
        criterion = criterion_dict['criterion']
        if args.center:
            criterion_center = criterion_dict['center']
            loss_type = os.path.join(loss_type, 'center')
        else:
            criterion_center = None
    elif args.triplet:
        if use_angular_loss:
            loss_type = "triplet/angular"
        else:
            loss_type = "triplet/ce"
        criterion = criterion_dict['criterion']
        criterion_similarity = criterion_dict['similarity']
        if args.center:
            print("use center")
    ...
    criterions = [criterion_similarity, criterion, criterion_center]
    ######################################################################
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train(
            model, linear_classifiers, optimizers, 
            criterions, train_loader, epoch, 
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            test_stats = validate_network(val_loader, model, linear_classifiers, criterions, args.n_last_blocks, args.avgpool_patchtokens, args.permutes)
            ...
            if utils.is_main_process() and (group_best_acc >= best_acc):
                ...
                torch.save(save_dict, os.path.join(args.output_dir, "checkpoint_{}_metric.pth".format(args.checkpoint_key)))
...
def train(model, linear_classifiers, optimizers, criterions, loader, epoch, n, avgpool, permutes):
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        with torch.no_grad():
            ...
            # print(f"vit out:{output[0].shape}")
            output = torch.cat(output, dim=-1)
            # print(f"vit concat out:{output.shape}")
        losses = []
        c_sim, ce, ccenter = criterions
        for linear_classifier, optimizer in zip(linear_classifiers, optimizers):
            if args.circle or args.triplet:
                pred, feat = linear_classifier(output)
            else:
            ...
            if args.circle:
                ...
                # Circle + Angular
                # Circle + Angular + Center
                if args.center:
                    center_loss = ccenter(feat, target)
                fnorm = torch.norm(feat, p=2, dim=1, keepdim=True)
                feat = feat.div(fnorm.expand_as(feat))
                loss += (c_sim(*convert_label_to_similarity(feat, target))/args.batch_size_per_gpu)
                if args.center:
                    loss += (args.center_weight * center_loss)
            elif args.triplet:
                # Including: 
                # Triplet + CE
                # Triplet + CE + Center
...
@torch.no_grad()
def validate_network(val_loader, model, linear_classifiers, criterions, n, avgpool, permutes):
    linear_classifiers.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
...
class LinearClassBlock(nn.Module):
    ...
    def forward(self, x):
        # print(f"lcb in:{x.shape}")
        x = self.block(x)
        # print(f"lcb out:{x.shape}")
        if self.return_f:
            f = x.clone()
            x = self.classifier(x)
            return [x,f]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--circle', action='store_true', help='use Circle loss + Classifier Loss' )
    parser.add_argument('--triplet', action='store_true', help='use Triplet loss + Classifier Loss')
    parser.add_argument('--triplet_margin', type=float, default=0.3, help='Margin of triplet loss (Only for triplet loss)' )
    parser.add_argument('--smooth', type=float, default=0., help='label smooth')  # TODO
    parser.add_argument('--angular_penalty', type=str, default=None, choices=['arcface', 'sphereface', 'cosface'])
    parser.add_argument('--use_backbone_feat', action='store_true', help='use backbone output for similarity loss, otherwise use the input of the classifier')
    parser.add_argument('--center_weight', type=float, default=0.0005, help='center loss weight')
