...
from loader import ImageFolderInstance, TrackletEmbeddingInstance
...
def bulid_dataloader(args):
    ...
    if 'rit' in args.arch:
        ...
        dataset_train = TrackletEmbeddingInstance(traindir, seq_len=args.num_patches)
        dataset_val = TrackletEmbeddingInstance(valdir, seq_len=args.num_patches)
    else:
...
def extract_feature_pipeline(args):
  ...
  # ============ preparing data ... ============
    data_loader_train, data_loader_val = bulid_dataloader(args)
    ...
    if args.dump_features and dist.get_rank() == 0:
        ...
        if os.path.exists(args.dump_features) is False:
...
@torch.no_grad()
def extract_features(model, data_loader, n, avgpool, use_cuda=True, multiscale=False, label_offset = 0):
...
@torch.no_grad()
def knn_classifier(train_features, train_labels, test_features, test_labels, k, T, use_cuda=True, num_classes=666):
