...
def eval_pred(label, pred, calc_acc=False):
    ...
    pred_adjusted = get_y_preds(label, pred, args.num_classes)
...
def bulid_dataloader(args):
    ...
    if 'rit' in args.arch:
        ...
        dataset_val = TrackletEmbeddingInstance(args.data_path, seq_len=args.num_patches, return_index=False)
    else:
        dataset_val = ImageFolder(args.data_path, transform=transform)
...
def main_eval(args):
    ...
    # ============ preparing data ... ============
    data_loader_val = bulid_dataloader(args)
    ...
    if args.checkpoint_key == "teacher":
        utils.restart_from_checkpoint(args.pretrained_weights, is_training=False, **{args.checkpoint_key: model})
    ...
    if 'rit' in args.arch:
        eval_unsup(model, data_loader_val, label_offset=1)
    else:
        eval_unsup(model, data_loader_val)


@torch.no_grad()
def eval_unsup(model, data_loader, label_offset=0):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_classes', default=666, type=int, help='Num of patches.')
