...
from loader import TrackletEmbeddingMask
from evaluation.unsupervised.unsup_cls import eval_pred

from .cam_framenum import CAM_FRAMENUM  #TODO:!!!
...
def get_args_parser():
    ...
    parser.add_argument('--num_patches', default=84, type=int, help="""len of tracklet sequence""")
    ...
    parser.add_argument('--shared_head', default=False, type=utils.bool_flag, help="""Wether to share 
        the same head for [CLS] token output and patch tokens output. When set to true, patch_out_dim
    ...
    # embedding fusion type
    parser.add_argument('--transform_type', default='standard', type=str, 
        choices=['reverse','multicam','concat_multicam',"concat_multicam_reverse"])
...
def train_ibot(args):
    ...
    # ============ preparing data ... ============
    # TODO: 把DataAug改成加embedding扰动，对抗训练

    args.global_crops_number = 2

    dataset = TrackletEmbeddingMask(
        args.data_path, 
    ...
    print(f"Data loaded: there are {len(dataset)} tracklets.")
    ...
    ibot_loss = iBOTLoss(
        ...
        args.global_crops_number,   # TODO:
        args.local_crops_number,    # TODO: 设置0 disable Multi-crop
    ...
    if args.pretrained_weights != "":
        utils.load_pretrained_weights(student, args.pretrained_weights, 'student', args.arch, args.patch_size, is_training=True)
        utils.load_pretrained_weights(teacher, args.pretrained_weights, 'teacher', args.arch, args.patch_size, is_training=True)
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    for it, input_data in enumerate(metric_logger.log_every(data_loader, 1, header)):
        ...
        sids, cids, labels, embeddings, masks = input_data['sid'], input_data['cid'], input_data['id'], input_data['input'], input_data['mask']
        
        # car-id starts from 1
        labels -= 1
        
        embeddings = [Variable(embed.cuda(non_blocking=True)) for embed in embeddings]
        masks = [Variable(msk.cuda(non_blocking=True)) for msk in masks]        
        
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            # TODO: Global View 和 Local View ？Multi-crop 是否需要？
            
            # get global views 
            teacher_output = teacher(embeddings[:args.global_crops_number])
            student_output = student(embeddings[:args.global_crops_number], mask=masks[:args.global_crops_number])
