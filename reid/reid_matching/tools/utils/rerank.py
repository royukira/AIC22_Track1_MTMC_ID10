...
def euclidean_distance(qf, gf, mask=None):
    m = qf.shape[0]
    n = gf.shape[0]
    ...
    # for L2-norm feature
    if mask is not None:
        dist_mat = 2 - 2 * torch.matmul(qf, gf.t()) * mask
    else:
...
def batch_torch_topk(qf, gf, k1, N=6000, mask=None):
    m = qf.shape[0]
    n = gf.shape[0]
    dist_mat = []
    initial_rank = []
    for j in range(n // N + 1):
        ...
        for i in range(m // N + 1):
            temp_qf = qf[i * N:i * N + N]
            temp_d = euclidean_distance(temp_qf, temp_gf, mask)
            temp_qd.append(temp_d)
        temp_qd = torch.cat(temp_qd, dim=0)
        temp_qd = temp_qd / (torch.max(temp_qd, dim=0)[0])
        temp_qd = temp_qd.t()
...
def ReRank2(probFea, galFea, k1=20, k2=6, lambda_value=0.3, mask=None):
    ...
    initial_rank = batch_torch_topk(feat, feat, k1 + 1, N=6000, mask=mask)
