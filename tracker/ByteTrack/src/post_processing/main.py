...
def load_raw_mot(seq, mcmt_cfg):
    ...
    in_dir = os.path.join(cfg.DATA_DIR, 'mot_result')
    mot_feat_dic = pickle.load(open(f'{in_dir}/{seq}/{seq}_mot_feat_raw.pkl', 'rb'))
    results = []
...
def eval_seq(seq, pp='', split='train', mcmt_cfg=None):
    ...
    in_dir = os.path.join(cfg.DATA_DIR, 'mot_result')
    trk_file = f'{in_dir}/{seq}/{seq}_mot.txt'
...
def save_pickle(results, sequence_name, mcmt_cfg):
    ...
    feat_pkl_file = f'{out_dir}/{sequence_name}/{sequence_name}_mot_feat.pkl'
    mot_feat_dic = {}
    for row in results:
